...
import { CARD_GAP, HEAD_GAP, SECTION, WRAP } from "@/lib/ui";
          ...
          {testimonials.items.map((t, i) => (
            <Reveal key={t.quote} delay={i * 70} className="h-full">
              ...
              <figure className="relative flex h-full flex-col justify-center rounded-3xl border border-line bg-white p-[clamp(24px,2.5vw,32px)] shadow-[var(--shadow-sm)] transition-[transform,box-shadow] duration-[280ms] ease-[cubic-bezier(0.22,0.7,0.2,1)] hover:-translate-y-1 hover:shadow-[var(--shadow)]">
                <blockquote className="text-pretty text-[clamp(1.1rem,1.02rem+0.35vw,1.28rem)] font-normal leading-[1.2] tracking-[-0.01em]">
                  &ldquo;{t.quote}&rdquo;
                </blockquote>
                ...
                <figcaption className="mt-2 flex items-center gap-[0.65em] font-mono text-[0.7rem] uppercase leading-[1.2] tracking-[0.07em] text-ink-faint before:h-0.5 before:w-5 before:flex-none before:rounded-sm before:bg-[image:var(--grad-ink)] before:content-['']">
                  {t.who}
                </figcaption>
              </figure>
            </Reveal>
